...
UserPaper = Table('UserPaper',Base.metadata,
    Column('id', Integer, primary_key=True),
    Column('UserId', Integer, ForeignKey('users.id')),
    Column('PaperId', Integer, ForeignKey('papers.id')),
)
...
class User(Base):
    ...
    papers = relationship("Paper", secondary=UserPaper, backref='Paper')
    __repr__ = f"USER:{username}-{id} Status:{active}\
        Read/Write/Mod/Adm:{read}-{write}-{mod}-{adm}"
class Paper(Base):
    ...
    users = relationship("User", secondary=UserPaper, backref='User')
